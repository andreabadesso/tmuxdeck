defmodule RelayWeb.AccountSettingsControllerTest do
  ...
  describe "GET /accounts/settings/confirm-email/:token" do
    ...
    test "updates the account email once", %{conn: conn, account: account, token: token, email: email} do
      conn = get(conn, ~p"/accounts/settings/confirm-email/#{token}")
      assert redirected_to(conn) == ~p"/accounts/settings"
    ...
    test "does not update email with invalid token", %{conn: conn, account: account} do
      conn = get(conn, ~p"/accounts/settings/confirm-email/oops")
      assert redirected_to(conn) == ~p"/accounts/settings"
